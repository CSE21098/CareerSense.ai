...
st.set_page_config(page_title="CareerSense.ai", page_icon='src/Logo College.png', layout='wide', initial_sidebar_state="expanded")
...
# Home Page Function
def home():
    st.markdown("<h1 style='text-align: center;'>Welcome to CareerSense.ai!</h1>", unsafe_allow_html=True)
    st.markdown("<h4 style='text-align: center;'>AI-powered System for Everyone</h4>", unsafe_allow_html=True)
    ...
    st.markdown("<a href='#tutorial' target='_blank'><button style='color: white; background-color: #4CAF50; border: none; padding: 10px 20px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px; margin: 4px 2px; cursor: pointer; border-radius: 12px;'>Tutorial</button></a>", unsafe_allow_html=True)
...
def main():
    ...
    with st.sidebar:
        ...
        tabs = on_hover_tabs(
            tabName=['Home', 'AI Lens','Career Guide', 'Ask To PDF', 'Resume Analyser', 'ATS','Contest Calendar','Job Tracker','Projects', 'Prompt Examples', 'About','Account'], 
            iconName=['home', 'center_focus_weak','school','search', 'article', 'work','calendar_month','work_outline','work_outline', 'edit', 'info','account_circle' ],  
            default_choice=0
        )
    ...
    st.markdown("<p style='text-align: center; color: #D3D3D3;'>- ©️GLBITM 2024 -</p>", unsafe_allow_html=True)
